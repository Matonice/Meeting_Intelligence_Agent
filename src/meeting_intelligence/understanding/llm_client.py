...
class LLMClient:
    ...
    def _complete_structured(self, messages: list[dict], response_model: type[T]) -> T:
        ...
        Tries native structured output first (gpt-4o+), falls back to JSON mode.
        """
        try:
            # Try native structured output (works with gpt-4o and later)
            ...
            logger.debug("Structured output not available, falling back to JSON mode")
